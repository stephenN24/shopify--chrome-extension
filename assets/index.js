...
const buildHTML = (info) => {
  ...
  let html = `<ul><h1>${logoSVG} ShopifyxBoostSD</h1>`;
  html += `<hr style="margin-bottom: 20px;border-color: #7fffd436;">`;
...
const prepareInfo = async (data) => {
  ...
  if (redirectlinks.current_page_JSON) {
    await constructAdminRedirectsLinks(
      ...
      if (response) {
        info.shopify.redirect.push(`Collection Admin->${response}`);
      }
    });
  }
